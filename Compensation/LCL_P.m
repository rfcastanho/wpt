%Equations for LCL P Compensation
%Note: Equations were test and compared with Pspice LCL_P file.
...
m = 1 - (Za/(1/(1i*w*Cp)+ Za));
c2 = Rs + 1i*w*(Ls-M)+ZL/(1 + 1i*w*ZL*Cs)+1i*w*M;
Ip = (m*Vp)/(c1 + (w^2*M^2)/c2);
Is = 1i*w*M*Ip/(c2);
VL = (ZL*Is)/(1 + 1i*w*ZL*Cs);
